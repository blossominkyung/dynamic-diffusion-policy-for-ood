...
from agent.helpers import (linear_beta_schedule,
...
class IR_SDE(SDEBase):
    def __init__(self, T=100, schedule='cosine', action_clip=False, device=None):
        ...
        if schedule == 'cosine':
            self.thetas = cosine_beta_schedule(T).to(device)
        elif schedule == 'linear':
            self.thetas = linear_beta_schedule(T).to(device)
        elif schedule == 'vp':
            self.thetas = vp_beta_schedule(T).to(device)
        elif schedule == 'sigmoid':
            self.thetas = sigmoid_beta_schedule(T).to(device)
        else:
    ...
    def log_forward_transition(self, x1, x2, t1, t2): # t1 < t2
        ...
        log_p = torch.log(2*math.pi * (1 - torch.exp(-2 * tb))) \
                + (x1 - x2 * torch.exp(-tb))**2 / (1 - torch.exp(-2 * tb) + 1e-8)   # 1e-8 추가
        return -0.5 * log_p
