...
hyperparameters = {
    ...
    'antmaze-umaze-v0':              {'lr': 3e-4, 'eta': 2.0,    'max_q_backup': False,    'reward_tune': 'cql_antmaze', 'num_epochs': 1000, 'gn': 2.0, 'loss_type': 'NML', 'action_clip': False},
    'antmaze-umaze-diverse-v0':      {'lr': 3e-4, 'eta': 1.0,    'max_q_backup': True,     'reward_tune': 'cql_antmaze', 'num_epochs': 1000, 'gn': 2.0, 'loss_type': 'NML', 'action_clip': False},
    'antmaze-medium-play-v0':        {'lr': 3e-4, 'eta': 2.0,    'max_q_backup': True,     'reward_tune': 'cql_antmaze', 'num_epochs': 1000, 'gn': 2.0, 'loss_type': 'NML', 'action_clip': False},
    'antmaze-medium-diverse-v0':     {'lr': 3e-4, 'eta': 2.0,    'max_q_backup': True,     'reward_tune': 'cql_antmaze', 'num_epochs': 1000, 'gn': 2.0, 'loss_type': 'NML', 'action_clip': False},
    'antmaze-large-play-v0':         {'lr': 3e-4, 'eta': 2.0,    'max_q_backup': True,     'reward_tune': 'cql_antmaze', 'num_epochs': 1000, 'gn': 2.0, 'loss_type': 'NML', 'action_clip': False},
    'antmaze-large-diverse-v0':      {'lr': 3e-4, 'eta': 2.0,    'max_q_backup': True,     'reward_tune': 'cql_antmaze', 'num_epochs': 1000, 'gn': 2.0, 'loss_type': 'NML', 'action_clip': False},
    
    'pen-human-v1':                  {'lr': 3e-5, 'eta': 0.1,    'max_q_backup': False,    'reward_tune': 'normalize',   'num_epochs': 1000, 'gn': 2.0, 'loss_type': 'NML', 'action_clip': True},
    'pen-cloned-v1':                 {'lr': 3e-5, 'eta': 0.1,    'max_q_backup': False,    'reward_tune': 'normalize',   'num_epochs': 1000, 'gn': 2.0, 'loss_type': 'NML', 'action_clip': True},
    
    'kitchen-complete-v0':           {'lr': 3e-4, 'eta': 0.005,   'max_q_backup': False,   'reward_tune': 'no',          'num_epochs': 1000, 'gn': 1.0, 'loss_type': 'MLL', 'action_clip': True},
    'kitchen-partial-v0':            {'lr': 3e-4, 'eta': 0.005,   'max_q_backup': False,   'reward_tune': 'no',          'num_epochs': 1000, 'gn': 1.0, 'loss_type': 'MLL', 'action_clip': True},
    'kitchen-mixed-v0':              {'lr': 3e-4, 'eta': 0.005,   'max_q_backup': False,   'reward_tune': 'no',          'num_epochs': 1000, 'gn': 1.0, 'loss_type': 'MLL', 'action_clip': True},
}
...
def train_agent(env, state_dim, action_dim, max_action, output_dir, args):
    ...
    if args.algo == 'agent':
        from agent.agent import DynamicDiffusionPolicy as Agent
        agent = Agent(state_dim=state_dim,
                      ...
                      subgoal_reg=args.subgoal_reg,
                      subgoals_cand=args.subgoal_cand,
                      risk_threshold=args.risk_threshold,
                      )
    elif args.algo == 'bc':
        from agent.bc_diffusion import Diffusion_BC as Agent
        agent = Agent(state_dim=state_dim,
                      action_dim=action_dim,
                      max_action=max_action,
                      device=args.device,
        ...
        from agent.ql_diffusion import Diffusion_QL as Agent
        agent = Agent(state_dim=state_dim,
                      action_dim=action_dim,
                      max_action=max_action,
                      device=args.device,
    ...
    while (training_iters < max_timesteps) and (not early_stop):
        ...
        logger.record_tabular('Score Penalty', np.mean(loss_metric['subgoal_penalty']))
        logger.dump_tabular()
        ...
        evaluations.append([
            ...
            np.mean(loss_metric['subgoal_penalty']),
            curr_epoch
        ])
...
def eval_policy(policy, env_name, seed, eval_episodes=10):
    ...
    utils.print_banner(f"Evaluation over {eval_episodes} episodes: avg_reward: {avg_reward:.2f}, avg_norm_score: {avg_norm_score:.2f}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--algo", default="agent", type=str)                    
    parser.add_argument('--device', default=0, type=int)                            
    parser.add_argument("--env_name", default="kitchen-complete-v0", type=str) 
    ...
    parser.add_argument("--lcb_coef", default=2, type=float)                      
    
    ### Subgoal Setting ###
    parser.add_argument("--rollout_steps", default=3, type=int)                   
    parser.add_argument("--subgoal_cand", default=3, type=int)                    
    parser.add_argument("--subgoal_reg", default=0.1, type=float)                 
    parser.add_argument("--risk_threshold", default=0.03, type=float)             
    parser.add_argument("--visualize", action='store_true', help="If set, do t-SNE after training.")
    ...
    file_name = f"ddp-{args.algo}|T-{args.T}"
    if args.lr_decay: file_name += '|lr_decay'
    file_name += f'|{args.seed}'
    if 'agent' in args.algo:
    ...
    variant = vars(args)
    variant.update(version=f"Dynamic-Diffusion-Policies")
